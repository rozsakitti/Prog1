...
struct Lines_window : Graph_lib::Window{ //a Lines_window a Window leszármazottja
    Lines_window(Point xy, int w, int h, const string& title); //konstruktor
    Open_polyline lines;
    ...
    private:
        ...
        void next(); //gombnyomáskor hívódik a next
        void quit();

        static void cb_red(Address, Address); //static: nemcsak objektumpéldány tagjaként szeretnénk használni
...
Lines_window::Lines_window(Point xy, int w, int h, const string& title)
    : Window{xy, w, h, title},
    next_button{Point{x_max() - 150, 0}, 70, 20, "Next point", //x_max=> az ablak szélessége, y_max a magassága
        [](Address, Address pw) { reference_to<Lines_window>(pw).next(); }}, //kasztolás | pw az a cím, ahol az ablakunk van,átadjuk a next-nek és átkasztoljuk egy Lines_window referencia típussá, aminek már hívhatjuk is a next tagját
...
ostringstream ss;
ss << '(' << x << ',' << y << ')';
xy_out.put(ss.str()); //kiíratjuk az output dobozba az x,y-t
redraw();
}

void Lines_window::cb_next(Address, Address pw) { //Address pw annak az ablaknak a címe,amibe hívjuk
...
int main(){
    try {
        ...
        return gui_main(); //elindítja a programot, fölcsatolgat etc.
    }
    catch(exception& e) {
        cerr << "Error: " << e.what() << '\n';
        return 1;
