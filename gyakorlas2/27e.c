#define MAX_TITKOS 4096 //4096 a méret
#define OLVASAS_BUFFER 256 //256 bájt a mérete
	...
	int titkos_meret, char *buffer)
{
    ...
    for (int i = 0; i < titkos_meret; ++i)
    {
        buffer[i] = titkos[i] ^ kulcs[kulcs_index]; //nem hatványozás, hanem exor művelet
        kulcs_index = (kulcs_index + 1) % kulcs_meret; //körbe-körbe a kulcsokon
    }
}

//titkosítás visszafejtő
void
...
int main(){
	...
	char kulcs[KULCS_MERET]; //karaktertömb a beolvasott kulcsokkal
	char titkos[MAX_TITKOS]; //titkosítandó szöveg
	char *p = titkos;
	int olvasott_bajtok;

	while ((olvasott_bajtok = 
		read ( 0, (void*) p, //addig megy míg a bemenet végére nem érünk
